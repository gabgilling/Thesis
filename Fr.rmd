...
eb81.3$nuts1 <- with(eb81.3, ifelse(isocntry == "FR", paste("FR", eb81.3$p7fr, sep = ""), 
                      ifelse(isocntry %in% c("GB-GBN", "GB-NIR"), paste("GB", eb81.3$p7gb, sep = ""),
                      ifelse(isocntry %in% c("DE-W", "DE-E"), paste("DE", eb81.3$p7de, sep = ""),
...
```{r}
library(data.table)
census2011 <- read.csv("/Users/gabgilling/Downloads/csvoutput_HC44_2020_09_25_01_12.csv")

census2011 <- census2011 %>% filter(SEX != 'T', AGE != 'TOTAL', !EDU %in% c('TOTAL','NONE', 'NAP', 'UNK') )

census2011$CAS <- NULL
census2011$TIME <- NULL
census2011$SIE <- NULL
census2011$LOC <- NULL
...
census2011$nuts1 <- census2011$GEO

unique(census2011$EDU)
```
