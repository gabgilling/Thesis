...
eb75.2$gender_male <- ifelse(eb75.2$v601 == 2, 1, 0)
...
eb81.3$protect.env <- ifelse(eb81.3$qa1 %in% c(1,2), 1, ifelse(eb81.3$qa1 %in% c(3,4), 0, NA))

eb75.2 %>% group_by(NUTS_eb) %>% summarise(mean(eu.leg, na.rm = T))

eb75.2$protect.env <- ifelse(eb75.2$v183 %in% c(1,2), 1, ifelse(eb75.2$v183 %in% c(3,4), 0, NA))
...
fit_var_logit <- function(survey_df = c(), var_name){
  ...
  if (length(survey_df) >= 2){
    ...
    survey_df = get(survey_df)
  }
  
  temp <- temp %>% group_by(age_cat2, gender_male, ms, NUTS_eb) %>%
  summarise(pro = sum(get(var_name) == 1), against = n() - pro) %>% na.omit()
  ...
  return(stan_glmer(cbind(pro, against) ~ (1|country) + (1|NUTS_eb) + (1 | age_cat2)  + (1|ms) + gdp_cap + higher_ed + hhd_income + unemp_pct + gender_male, data = temp, family = binomial(link = "logit"), refresh= 0))

}

set.seed(1010)

fit.protect.env <- fit_var_logit(survey_df = c("eb81.3", "eb75.2"), var_name = "protect.env")
fit.eu.enf <- fit_var_logit(survey_df = "eb81.3", var_name = "eu.enf")
fit.eu.leg <- fit_var_logit(survey_df = c("eb81.3", "eb75.2"), var_name = "eu.leg")
fit.eu.asst <- fit_var_logit(survey_df = c("eb81.3", "eb75.2"), var_name = "eu.asst")
fit.eu.fund <- fit_var_logit(survey_df = c("eb81.3", "eb75.2"), var_name = "eu.fund")
fit.cooperation <- fit_var_logit(survey_df = c("eb81.3", "eb75.2"), var_name = "cooperation")
fit.envorecon <- fit_var_logit(survey_df = c("eb81.3", "eb75.2"), var_name = "envorecon")
