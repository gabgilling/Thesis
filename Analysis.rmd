...
eb81.3$NUTS <- with(eb81.3, ifelse(isocntry == "FR", paste("FR", eb81.3$p7fr, sep = ""), 
                      ifelse(isocntry %in% c("GB-GBN", "GB-NIR"), paste("UK", eb81.3$p7gb, sep = ""),
                      ifelse(isocntry == "IT", paste("IT", eb81.3$p7it_r1, sep = ""), NA))))
...
census2011_education <- merge(reg_preds %>% dplyr::select(NUTS2_old, NUTS),census2011_education, by = "NUTS2_old")
