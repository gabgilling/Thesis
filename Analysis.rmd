...
fit.protect.env <- stan_glmer(cbind(pro, against) ~ (1|country) + (1|NUTS_eb) + (1 | age_cat)  + (1|edu) + gdp_cap + higher_ed + hhd_income + unemp_pct + gender_male, data = protect.env, family = binomial(link = "logit"))
...
env_prefs_plot <- ggplot(protect.envs_prefs.r, aes(x= constituency, y = pref, group = 1, color = country)) + geom_point() + 
...
mep.ids$ideo6 <- with(mep.ids, ifelse(Group %in% c("Alliance of Conservatives and Reformists in Europe", "European People's Party" ), "Right", ideo6))

unique(mep.ids[mep.ids$ideo6 == "",]$Group)
