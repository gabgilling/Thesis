...
```{r}
ggplot(mep.ids %>% group_by(constituency, Parliament) %>% summarise(mep_pref = mean(env1+env2), mep_se = sd(env1+env2) / sqrt(length(env1+env2))) %>% na.omit(), aes(x = constituency, y = mep_pref)
```
