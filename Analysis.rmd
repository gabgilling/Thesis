...
census2011_education <- merge(reg_preds %>% dplyr::select(NUTS2_old, NUTS, NUTS_eb),census2011_education, by = "NUTS2_old")
...
nat_resources <- merge(nat_resources, reg_preds, by = "NUTS_eb")



fit.nat_resources <- stan_glmer(cbind(pro, against) ~ (1|NUTS_eb) + (1|gender_male)  + (1 | age_cat)  + (1|edu) + p_tot_catholic_adh + p_evan_mormon_adh, data = sup.money, family = binomial(link = "logit"), 
