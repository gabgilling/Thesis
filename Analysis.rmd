...
nat_resources$country <- substr(nat_resources$NUTS, 1, 2)

fit.nat_resources <- stan_glmer(cbind(pro, against) ~ (1|country) + (1|NUTS_eb) + (1|gender_male)  + (1 | age_cat)  + (1|edu) + gdp_cap + higher_ed + hhd_income + unemp_pct, data = nat_resources, family = binomial(link = "logit"))
...
generate_region_estimates <- function(poststrat, fitted_model){
  ...
  for (i in unique(poststrat$Region.Name)) {
    ...
    region_preferences[region_preferences$region == i,]$se <- round(sd(poststrat_prob_region), 4)
    region_preferences[region_preferences$region == i,]$country <- unique(poststrat_region$country)
  }
...
ggplot(nat_resources_prefs, aes(x= region, y = pref, group = 1, color = country)) + geom_point() + 
...
Link regions to constituencies
