...
# need to subset by term

for (i in env2$votes$`+`$groups){
  for (id in unname(unlist(i))){
    if (id %in% unique(mep.ids$ID)){
      mep.ids[mep.ids$ID == id,]$env2 <- 1
    }
  }
}
...
count_votes(resolution_name = "A8.0249", "env2")
count_votes(resolution_name = "A8.0354", "env1")

```


```{r}
mep.ids %>% group_by(constituency) %>% summarise(mep_pref = mean())
```
