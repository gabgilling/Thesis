...
## need to create a constitutency variable

Step 2: Census
```{r}
library(data.table)

nuts_dictionary <- read.csv("https://raw.githubusercontent.com/gabgilling/Thesis/master/nuts_dictionary.csv")

census2011_education <- read.csv("https://raw.githubusercontent.com/gabgilling/Thesis/master/census_education.csv")

census2011_education <- census2011_education %>% filter(SEX != 'T', AGE != 'TOTAL', !EDU %in% c('TOTAL','NONE', 'NAP', 'UNK') )
...
for (i in 1:nrow(census2011_education)){
  census2011_education$constituency[i] <- ifelse(census2011_education$GEO[i] == nuts_dictionary$NUTS1[i]
                                                 |census2011_education$GEO[i] == nuts_dictionary$NUTS2[i], 
                                                 nuts_dictionary$CONSTITUENCY[i], NA)
}
census2011_education$constituency <- ifelse(census2011_education$GEO == nuts_dictionary$NUTS1, nuts_dictionary$CONSTITUENCY, NA)


census2011$age_cat <- ifelse(census2011$AGE %in% c('Y18', 'Y19', 'Y20-24'), 'Y18-24', census2011$AGE)
census2011$gender_male <- ifelse(census2011$SEX == 'M', 1,0 )
census2011$edu <- with(census2011, ifelse(EDU == 'ED1', 1, ifelse(EDU == 'ED2', 2, ifelse(EDU == 'ED3', 3,
                                    ifelse(EDU == 'ED4', 4, ifelse(EDU == 'ED5', 5, ifelse(EDU == 'ED6', 6, EDU)))))))

unique(census2011$nuts1)
```


Modeling 

NEED REGIONAL LEVEL PREDICTORS?
